...
//    LOGD("DrawResult::Reprojection");
    ...
    {
        Eigen::Vector3f Pc;
        VINS_LOG("world_position=%f %f %f",pose[i].x(),pose[i].y(),pose[i].z());
    ...
    //draw existing boxes
    LOGD("xxxx draw existing boxes");
    if (box_in_trajectorty)
        LOGD("xxxx drawBoxVirturCam");
