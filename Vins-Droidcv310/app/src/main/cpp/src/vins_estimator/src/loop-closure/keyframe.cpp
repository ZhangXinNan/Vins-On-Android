...
void KeyFrame::extractBrief(cv::Mat &image)
{
    ...
    VINS_LOG("descriptor size = %d",measurements.size());
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
   ...
   // LOGD("t_pnp_ransac %f ms", t_pnp_ransac.toc());
                                          ...
                                          const camodocal::CameraPtr &m_camera)
{
   ...
   // LOGD("loop final use num %d %lf---------------", (int)measurements_old.size(), t_match.toc());
    return true;
}
...
void BriefExtractor::operator() (const cv::Mat &im, const std::vector<cv::Point2f> window_pts,
                                 vector<cv::KeyPoint> &keypoints, vector<BRIEF::bitset> &descriptors) const
{
  ...
  m_brief.compute(im, keypoints, descriptors);
}